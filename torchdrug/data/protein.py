...
class PackedProtein(PackedMolecule, Protein):
    ...
    @classmethod
    def _residue_from_sequences(cls, sequences):
        num_residues = []
        residue_type = []
        residue_feature = []
    ...
    @classmethod
    @utils.deprecated_alias(node_feature="atom_feature", edge_feature="bond_feature", graph_feature="mol_feature")
    def from_sequences(cls, sequences, atom_feature="default", bond_feature="default", residue_feature="default",
                      mol_feature=None, kekulize=False):
        """
        ...
        """
        if atom_feature is None and bond_feature is None and residue_feature == "default":
            return cls._residue_from_sequences(sequences)
    ...
    def residue2graph(self):
        """Residue id to graph id mapping."""
        # if self.batch_size != self.num_residues:
        # breakpoint()
        range = torch.arange(self.num_residues.shape[0], device=self.device)
