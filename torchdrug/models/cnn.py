...
class ProteinResNet(nn.Module, core.Configurable):
    ...
    def forward(self, graph, input, mask=None, all_loss=None, metric=None, ig=False):
        """
        ...
        if hidden.shape[0] != graph.num_residues.shape[0]:
            repeat_times = hidden.size(0) // graph.num_residues.size(0)
            num_residues = graph.num_residues.repeat(repeat_times)
            residue_feature = functional.padded_to_variadic(hidden, num_residues)
            graph_tmp = copy.deepcopy(graph)
            graph_tmp.num_residues = num_residues
            graph_feature = self.readout(graph_tmp, residue_feature)
        else:
...
class ProteinConvolutionalNetwork(nn.Module, core.Configurable):
    ...
    def forward(self, graph, input, all_loss=None, metric=None, ig=False):
        """
        Compute the residue representations and the graph representation(s).
