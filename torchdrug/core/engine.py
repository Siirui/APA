...
class Engine(core.Configurable):
    ...
    def train(self, num_epoch=1, ig=False, batch_per_epoch=None, policy=None):
        """
        Train the model.
        ...
        for epoch in self.meter(num_epoch):
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                ...
                # torch.save(batch, '/hanpengfei/fixed_batch.pth')
                # print(batch_id)
                # if batch_id % 10 == 0:
                    # breakpoint()
                sub_policy = None
                ...
                if policy is not None:
                    loss, metric = model(batch, ig, sub_policy)
                else:
                    loss, metric = model(batch, True, sub_policy)
                # else:
                    # loss, metric = model(batch, False, policy)
